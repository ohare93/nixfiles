    ...
    programs.niri.settings = {
      ...
      outputs = {
        ...
        "eDP-1" = {
          ...
          position = {
            x = 0;
            y = 1152;
          }; # Below external (1440/1.25 = 1152)
          scale = 1.25;

          # Laptop gets wider defaults than the ultrawide profile.
          layout = {
      ...
      spawn-at-startup = [
        ...
        }
        {
          command = [
            "sh"
            "-c"
            "~/.local/bin/niri-display-switcher.sh"
          ];
        } # Initialize display state on login
      ];
      ...
      binds = {
        ...
        "Mod+Shift+Q".action.spawn = [
          "sh"
          "-c"
          "systemctl --user start niri-session-quit.service"
        ];
        ...
        "Mod+Shift+Escape".action.spawn = [
          "sh"
          "-c"
          "~/.local/bin/niri-toggle-display.sh"
        ];
        ...
        "Mod+Print".action.spawn = [
          "sh"
          "-c"
          "grim -o $(niri msg focused-output) /tmp/screenshots/screenshot-$(date +%Y-%m-%d-%H-%M-%S).png"
        ];
        "Mod+Shift+Print".action.spawn = [
          "sh"
          "-c"
          "grim -g \"$(slurp)\" /tmp/screenshots/screenshot-$(date +%Y-%m-%d-%H-%M-%S).png"
        ];
